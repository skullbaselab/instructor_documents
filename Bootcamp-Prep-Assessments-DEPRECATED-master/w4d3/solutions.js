...
// Answer 3 - Object.values() is experimental ES2017
Object.prototype.values = function(){
    return Object.keys(this).map(x => this[x]);
};

function concatSandwich(wd){
    return function(){
        return arguments.values().join(` ${wd} `).trim();
    }
}
...
/* Answer:
function countAdjacentSums(arr, tar){
...
/*
Answer:
function isogramMatcher(s1, s2){
    return s1
    .split('')
    .reduce((p, c, i) => {
        if (s2[i] === c)    p[0]++;
        else if (s2.includes(c)) p[1]++;
